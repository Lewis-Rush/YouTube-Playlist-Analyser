...
class TestMain:
    ...
    def test_main_gets_api_key(self, monkeypatch):
        monkeypatch.setattr("builtins.input", lambda prompt: "0" if "videos watched" in prompt else "https://test.com")

        mock_api_key = "test_key"
        monkeypatch.setattr("src.main.get_api_key", lambda: mock_api_key)

        with patch("src.main.get_playlist") as mock_get_playlist, \
            patch("src.main.no_videos_watched") as mock_no_videos, \
            patch("src.main.has_watched_videos") as mock_has_watched, \
    ...
    def test_main_gets_user_input_correctly(self, monkeypatch):
        ...
        mock_get_playlist.assert_called_once_with("https://test.com", mock_youtube_client)
